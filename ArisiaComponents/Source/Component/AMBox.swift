...
public class AMBox: KCStackView, ALFrame
{
	...
	static private var baseInterfaceType: CNInterfaceType { get {
		...
		if let iftype = vmgr.searchInterfaceType(byTypeName: ifname) {
			...
			let ptypes: Array<M> = [
				M(name: AMBox.AxisItem,		type: .enumType(CNAxis.allocateEnumType())),
				M(name: AMBox.AlignmentItem,	type: .enumType(CNAlignment.allocateEnumType())),
				M(name: AMBox.DistributionItem,	type: .enumType(CNDistribution.allocateEnumType()))
			]
	...
	public func setup(path pth: ALFramePath, resource res: KEResource, console cons: CNFileConsole) -> NSError? {
		...
		/* Set property types */
		defineInterfaceType(interfaceType: AMBox.baseInterfaceType)
