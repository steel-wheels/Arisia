...
public class ALParser
{
	...
	private func parseElementType(stream strm: CNTokenStream, sourceFile srcfile: URL?) -> Result<CNValueType, NSError> {
		if let rword = requireReservedWord(stream: strm) {
			...
			if let etype = vmgr.searchEnumType(byTypeName: ident) {
				return .success(.enumType(etype))
			}
			/* Decode as interface */
			if let iftype = vmgr.searchInterfaceType(byTypeName: ident) {
				return .success(.interfaceType(iftype))
			}
			/* Decode as frame */
			if ALFrameManager.shared.isFrameClassName(name: ident) {
	...
	private func parseValue(valueType vtype: CNValueType, stream strm: CNTokenStream, sourceFile srcfile: URL?) -> Result<ALValueIR, NSError> {
		...
		case .numberType:
			...
			if let val = strm.requireNumber() {
				let newval = hasplus ? val : val.negating()
				return .success(.number(newval))
			} else if let txt = strm.requireText() {
				let ifunc = ALImmediatelyInvokedFunctionIR(returnType: vtype, script: txt, source: nil, config: mConfig)
				return .success(.invokedFunction(ifunc))
			} else {
		...
		case .interfaceType(let iftype):
			switch parseInterfaceValue(interfaceType: iftype, stream: strm, sourceFile: srcfile) {
		...
		case .functionType(_, _), .nullable(_):
	...
	private func parseInterfaceValue(interfaceType iftype: CNInterfaceType, stream strm: CNTokenStream, sourceFile srcfile: URL?) -> Result<ALValueIR, NSError> {
		if strm.requireSymbol(symbol: "{") {
			var result: Dictionary<String, ALValueIR> = [:]
			var is1st:  Bool = true
			while true {
				...
				if let key = strm.requireIdentifier() {
					if let _ = iftype.type(for: key) {
						...
						return .failure(parseError(message: "Unknown member \"\(key)\" for interface \"\(iftype.name)\"", stream: strm))
					}
				} else {
					return .failure(parseError(message: "identifier is required as dictionary key", stream: strm))
				}
		...
		} else if let txt = strm.getText() {
			let vtype: CNValueType = .interfaceType(iftype)
			let ifunc = ALImmediatelyInvokedFunctionIR(returnType: vtype, script: txt, source: nil, config: mConfig)
			return .success(.invokedFunction(ifunc))
		} else {
			return .failure(parseError(message: "interface value is required", stream: strm))
		}
	}
	...
	private func makeNearInfo(stream strm: CNTokenStream?) -> String {
		...
		if let stm = strm {
			if let token = stm.get() {
				nearinfo = "near token \(token.code(withComment: true)) "
			}
		}
		return nearinfo
	}
	...
	private func dumpTokens(stream strm: CNTokenStream) {
		...
		while let tkn = strm.peek(offset: index) {
			CNLog(logLevel: .error, message: "tkn\(index): \(tkn.code(withComment: true))")
			index += 1
		}
	}
}
