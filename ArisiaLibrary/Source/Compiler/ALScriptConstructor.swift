...
public class ALScriptConstructor
{
	...
	public func construct(frame frm: ALFrameIR) -> Result<CNTextSection, NSError> {
		...
		switch insertSetupper(frame: frm, instanceName: ALConfig.rootInstanceName) {
		...
		switch insertListnerCallers(frame: frm, instanceName: ALConfig.rootInstanceName, pathStack: CNStack()) {
		case .success(let txt):
			result.add(text: txt)
		case .failure(let err):
			return .failure(err)
		...
		switch insertImmediatelyInvokedFunctions(frame: frm, instanceName: ALConfig.rootInstanceName, pathStack: CNStack()) {
		case .success(let txt):
			result.add(text: txt)
		case .failure(let err):
			return .failure(err)
		...
		switch insertInitCallers(frame: frm, instanceName: ALConfig.rootInstanceName, pathStack: CNStack()) {
		case .success(let txt):
			result.add(text: txt)
		case .failure(let err):
			return .failure(err)
		...
		result.add(text: CNTextLine(string: "\(ALConfig.rootInstanceName) ;"))
	...
	public func insertSetupper(frame frm: ALFrameIR, instanceName iname: String) -> Result<CNTextSection, NSError> {
		let result = CNTextSection()
		result.add(text: CNTextLine(string: "/* Setup the component */"))
		result.add(text: CNTextLine(string: "\(ALLibraryCompiler.SetupComponentFuncName)(\(iname)) ;"))
		return .success(result)
	}
	...
	public func insertImmediatelyInvokedFunctions(frame frm: ALFrameIR, instanceName iname: String, pathStack paths: CNStack<String>) -> Result<CNTextSection, NSError> {
		let result = CNTextSection()
		paths.push(iname)
		...
		for pname in frm.propertyNames {
			if let pval = frm.value(name: pname) {
				switch pval {
				case .frame(let child):
					switch insertImmediatelyInvokedFunctions(frame: child, instanceName: pname, pathStack: paths) {
		...
		/* Visit the frame itself */
		let pathelms = paths.peekAll(doReverseOrder: false)
		for pname in frm.propertyNames {
			if let pval = frm.value(name: pname) {
				switch pval {
				case .invokedFunction(let ifunc):
					let pstr = pathelms.joined(separator: ".")
					let line = ifunc.toScript(language: .JavaScript)
					result.add(text: CNTextLine(string: pstr + "." + pname + " = " + line + " ;"))
				default:
					break
				}
			}
