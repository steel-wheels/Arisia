...
open class ALFunctionIR
{
	...
	open func toScript(returnType rettype: CNValueType, language lang: ALLanguage) -> String {
		return toScript(arguments: [], returnType: rettype, preStatements: "", language: lang)
	}

	public func toScript(arguments args: Array<ALArgument>, returnType rettype: CNValueType, preStatements pstmts: String, language lang: ALLanguage) -> String {
	...
	public func selfArgument() -> ALArgument {
		if let frm = self.owner {
			let stype = ALFunctionInterface.userInterfaceType(path: frm.path)
			return ALArgument(type: stype, name: "self")
		} else {
	...
	public func selfPathArgument() -> ALPathArgument {
		let argtype: CNValueType
		if let frm = self.owner {
			argtype = ALFunctionInterface.userInterfaceType(path: frm.path)
		} else {
			argtype = .objectType(ALFunctionInterface.FrameInterface)
		}
...
public class ALInitFunctionIR: ALFunctionIR
{
	...
	public func toScript(language lang: ALLanguage) -> String {
		return super.toScript(arguments: [self.selfArgument()], returnType: .voidType, preStatements: "", language: lang)
	}

	public override func toType(framePath path: ALFramePath) -> CNValueType {
		let argtype = ALFunctionInterface.userInterfaceType(path: path)
		return .functionType(.voidType, [argtype])
	}
}
...
public class ALEventFunctionIR: ALFunctionIR
{
	...
	public func toScript(language lang: ALLanguage) -> String {
		...
		return super.toScript(arguments: args, returnType: .voidType, preStatements: "", language: lang)
	}

	public override func toType(framePath path: ALFramePath) -> CNValueType {
		let selftype = ALFunctionInterface.userInterfaceType(path: path)
...
public class ALProceduralFunctionIR: ALFunctionIR
{
	...
	public func toScript(language lang: ALLanguage) -> String {
		/* insert "self" definition */
		let prestmts: String
		if let frm = self.owner {
			var path = frm.path.path.joined(separator: ".")
			if let inst = frm.path.instanceName {
				path += "." + inst
			}
			prestmts = "\tlet self = \(path) ;"
		} else {
			prestmts = ""
		}
		return super.toScript(arguments: mArguments, returnType: mReturnType, preStatements: prestmts, language: lang)
	}
