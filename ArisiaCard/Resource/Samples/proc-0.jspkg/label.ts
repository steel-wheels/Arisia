...
_definePropertyIF(root, ["label","set_button","ok_button","axis","alignment","distribution"]) ;
{
  ...
  label._definePropertyType("set_text", "f(v,[s])") ;
  label._definePropertyType("number", "n") ;
  /* define getter/setter for all properties */
  _definePropertyIF(label, ["text","set_text","number"]) ;
  /* assign user declared properties */
  label.text = "Hello, World !!";
  label.set_text = function(txt: string): void {
  let self = root.label ;
        self.text = txt ;
      };
  root.label = label ;
}
{
  ...
  let set_button = _alloc_Button() as root_set_button_ButtonIF  ;
  /* define type for all properties */
  ...
  set_button.pressed = function(self: root_set_button_ButtonIF): void {
